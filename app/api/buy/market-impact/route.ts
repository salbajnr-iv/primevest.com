  ...
  try {
    ...
    if (!body || typeof body !== "object") {
      return NextResponse.json(
        { ok: false, error: "Request payload must be a JSON object." },
        { status: 400 },
      );
    }

    const amountEur = Number((body as Record<string, unknown>).amountEur);
    const liquidityEur = Number(
      (body as Record<string, unknown>).liquidityEur ?? DEFAULT_LIQUIDITY_EUR,
    );
    const symbol = String(
      (body as Record<string, unknown>).symbol ?? "",
    ).toUpperCase();

    const safeAmountEur =
      Number.isFinite(amountEur) && amountEur > 0 ? amountEur : 0;
    const safeLiquidityEur =
      Number.isFinite(liquidityEur) && liquidityEur > 0
        ? liquidityEur
        : DEFAULT_LIQUIDITY_EUR;
    ...
    try {
      ...
      if (!error) {
        ...
        ) as RpcMarketImpactResult | null;
      }
    } catch {
      rpcResult = null;
    }

    const impactPct = Number(
      rpcResult?.impact_pct ??
        calculateMarketImpactPercent({
    ...
    const classification =
      rpcResult?.classification ??
      (blocked
        ? "blocked"
        : requiresConfirmation
          ? "confirm_required"
          : impactPct >= MARKET_IMPACT_THRESHOLDS.warnPct
            ? "warn"
            : "normal");
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        amountEur: safeAmountEur,
        liquidityEur: safeLiquidityEur,
        impactPct: Number.isFinite(impactPct)
          ? Number(impactPct.toFixed(2))
          : 0,
        severity,
        classification,
        threshold: {
          warnPct: Number(
            rpcResult?.warn_threshold ?? MARKET_IMPACT_THRESHOLDS.warnPct,
          ),
          confirmationRequiredPct: Number(
            rpcResult?.confirmation_threshold ??
              MARKET_IMPACT_THRESHOLDS.confirmationRequiredPct,
          ),
          blockPct: Number(
            rpcResult?.block_threshold ?? MARKET_IMPACT_THRESHOLDS.blockPct,
          ),
        },
        requiresConfirmation,
        blocked,
    ...
    return NextResponse.json(
      {
        ok: false,
        error: "Failed to estimate market impact.",
