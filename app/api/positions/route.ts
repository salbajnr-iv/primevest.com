...
const SUPPORTED_SORTS = new Set<PositionsSortKey>([
  "allocation",
  "value",
  "pnl",
]);
...
export async function GET(req: Request) {
  try {
    ...
    let dbQuery = supabase
      .from("positions")
      .select(
        "id,asset,quantity,market_value,unrealized_pnl,avg_cost,updated_at,assets:asset_id(symbol,name)",
        { count: "exact" },
      )
      .eq("user_id", user.id);
    ...
    if (query) {
      ...
      dbQuery = dbQuery.or(
        `asset.ilike.%${escaped}%,assets.symbol.ilike.%${escaped}%,assets.name.ilike.%${escaped}%`,
      );
    }

    if (sort === "pnl") {
      dbQuery = dbQuery.order("unrealized_pnl", {
        ascending: direction === "asc",
        nullsFirst: false,
      });
    } else {
      dbQuery = dbQuery.order("market_value", {
        ascending: direction === "asc",
        nullsFirst: false,
      });
    }
    ...
    const positions = rows.map((row) => ({
      ...
      allocationPct:
        totalValue > 0
          ? Number(((row.value / totalValue) * 100).toFixed(4))
          : 0,
    }));
